...
class _PairDataset(torch.utils.data.Dataset):
    ...
    def sample_new_items(self, seed):
        ...
        if split != "train" and self.conf[split + "_pairs"] is not None:
            ...
            # train or validation with no fixed pairs
            for scene in self.scenes:
                path = self.info_dir / (scene + ".npz")
                ...
                if num_pos is not None:
                    ...
                    # we control the overlap set: [0.1-0.2, 0.2-0.3, 0.3-0.7]
                    bin_width = [0.1, 0.1, self.conf.max_overlap - self.conf.min_overlap - 0.2]

                    pairs_all = []
                    bin_min = self.conf.min_overlap
                    for k in range(num_bins):
                        # pairs_all list contains multi bins, each bin contains pairs
                        bin_max = bin_min + bin_width[k]
                    ...
                    "Get all pairs in the range of min_overlap and max_overlap."
        ...
        #         scene, i0, i1, overlap = self.items[idx]
        #         bin_width = (self.conf.max_overlap - self.conf.min_overlap) / 6  # bin_width = 0.1
        #         # count the number of items in each bin
        #         bin_index = int((overlap-0.0001 - self.conf.min_overlap) // bin_width)
        #         counter[bin_index] += 1
        #     logger.info("The number of items in each bin: %s", counter)
    ...
    def sample_new_items(self, seed):
        ...
        if split != "train" and self.conf[split + "_pairs"] is not None:
            if Path(self.conf[split + "_pairs"]).exists():
                ...
                pairs_path = MEGA_DATA_PATH / "configs" / self.conf[split + "_pairs"]
            for line in pairs_path.read_text().rstrip("\n").split("\n"):
