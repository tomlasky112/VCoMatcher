...
from src.models.cache_loader import CacheLoader, pad_local_features
...
from src.datasets.augmentations import IdentityAugmentation, augmentations
...
class MultiviewHomographyDataset(BaseDataset):
    default_conf = {
        # image search
...
class _Dataset(torch.utils.data.Dataset):
    ...
    def getitem(self, idx):
        ...
        # target view
        target_view = self._read_view(img, left_conf, ps, left=True)

        # source views
        num_source = self.conf.num_source
        source_views = []
        for i in range(num_source):
            source_views.append(self._read_view(img, self.conf.homography, ps, left=False))

        # compute Ht2s
        Hs = []
        for i in range(num_source):
            Hs.append(compute_homography(target_view["coords"], source_views[i]["coords"], [1, 1]).astype(np.float32))
...
def visualize(args):
    ...
    dataset = MultiviewHomographyDataset(conf)
