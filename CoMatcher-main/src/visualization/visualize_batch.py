...
def multiview_make_match_figures(pred_, data_, n_pairs=2):
    ...
    target_view, source_views = data["target_view"], data["source_views"]

    n_pairs = min(n_pairs, target_view["image"].shape[0])
    assert target_view["image"].shape[0] >= n_pairs

    kp0, kp1 = pred["target_keypoints"], pred["source_keypoints"][:, 0, :, :]
    m0 = pred["t2s_matches"][:, 0, :]
    gtm0 = pred["gt_t2s_matches"][:, 0, :]

    for i in range(n_pairs):
        valid = (m0[i] > -1) & (gtm0[i] >= -1)
        kpm0, kpm1 = kp0[i][valid].numpy(), kp1[i][m0[i][valid]].numpy()
        images.append(
            [target_view["image"][i].permute(1, 2, 0), source_views["image"][:, 0, :, :, :][i].permute(1, 2, 0)]
        )
        ...
        #         ]
        #     )
        # elif "depth" in target_view.keys() and target_view["depth"] is not None:
        #     heatmaps.append([target_view["depth"][i], source_views[:, 0, :, :]["depth"][i]])
