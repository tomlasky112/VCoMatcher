...
class EvalPipeline:
    ...
    def run(self, experiment_dir, model=None, overwrite=False, overwrite_eval=False, save_fig=False):
        ...
        if not exists_eval(experiment_dir) or overwrite_eval or overwrite:
            s, f, r = self.run_eval(self.get_dataloader(), pred_file, save_fig=save_fig)
