...
from src.utils.export_predictions import export_predictions
...
class MegaDepth4500Pipeline(EvalPipeline):
    default_conf = {
        "data": {
            "name": "image_pairs",
        ...
        "eval": {
            "estimator": "opencv",
            "ransac_th": 0.5,  # -1 runs a bunch of thresholds and selects the best
        },
    }
...
if __name__ == "__main__":
    from src import logger  # overwrite the logger

    dataset_name = Path(__file__).stem
    parser = get_eval_parser()
    args = parser.parse_intermixed_args()

    default_conf = OmegaConf.create(MegaDepth4500Pipeline.default_conf)
    ...
    pipeline = MegaDepth4500Pipeline(conf)
    s, f, r = pipeline.run(
        experiment_dir,
