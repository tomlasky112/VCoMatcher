...
@torch.no_grad()
def multiview_gt_matches_from_pose_depth(
    kp0, kp1, camera0, camera1, T_0to1, T_1to0, depth0, depth1, pos_th=3, neg_th=5, epi_th=None, cc_th=None, **kw
):
    if kp0.shape[1] == 0 or kp1.shape[1] == 0:
        b_size, n_kp0 = kp0.shape[:2]
        n_kp1 = kp1.shape[1]
    ...
    return {
        ...
        "t2s_matches": m0,
        "s2t_matches": m1,
        "t2s_matching_scores": (m0 > -1).float(),
        "s2t_matching_scores": (m1 > -1).float(),
        "depth_keypoints_t": d0,
        "depth_keypoints_s": d1,
        "proj_t2s": kp0_1,
        "proj_s2t": kp1_0,
        "visible_t": visible0,
        "visible_s": visible1,
...
@torch.no_grad()
def gt_matches_from_homography(kp0, kp1, H, pos_th=3, neg_th=6, multiview=False, **kw):
    if kp0.shape[1] == 0 or kp1.shape[1] == 0:
        b_size, n_kp0 = kp0.shape[:2]
        n_kp1 = kp1.shape[1]
        assignment = torch.zeros(
