...
@torch.no_grad()
def multiview_matcher_metrics(pred, data, prefix="", prefix_gt=None):
    def recall(m, gt_m):
        mask = (gt_m > -1).float()
        return ((m == gt_m) * mask).sum(2) / (1e-8 + mask.sum(2))
    ...
    if prefix_gt is None:
        prefix_gt = prefix
    rec = recall(pred[f"{prefix}t2s_matches"], data[f"gt_{prefix_gt}t2s_matches"])
    prec = precision(pred[f"{prefix}t2s_matches"], data[f"gt_{prefix_gt}t2s_matches"])
    acc = accuracy(pred[f"{prefix}t2s_matches"], data[f"gt_{prefix_gt}t2s_matches"])
    ap = ranking_ap(
        pred[f"{prefix}t2s_matches"],
        data[f"gt_{prefix_gt}t2s_matches"],
        pred[f"{prefix}t2s_matches_scores"],
    )
    metrics = {
        f"{prefix}match_recall": rec.mean(dim=1),
        f"{prefix}match_precision": prec.mean(dim=1),
        f"{prefix}accuracy": acc.mean(dim=1),
        f"{prefix}average_precision": ap.mean(dim=1),
    }
    return metrics
